...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  // 🟢 1. User Identity (Syncing with Session/DB)
  const [user, setUser] = useState<UserProfile | null>({
      name: "System Admin",
      organization: "Sakura Group",
      email: "admin@sakuragroup.co.tz",
      phoneNumber: null, // Track for Node Strength
      balance: 0,
      role: "ADMIN"
  });

  // 🟢 2. Functional Purity: Node Strength Calculation
  ...
  useEffect(() => {
    async function syncUserData() {
      try {
        const res = await fetch("/api/sms/balance"); // Your existing balance endpoint
        if (res.ok) {
           const data = await res.json();
           setUser(prev => prev ? { ...prev, balance: data.balance, phoneNumber: data.phoneNumber } : prev);
        }
      } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans selection:bg-pink-100 flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 fixed h-full z-20 shadow-[2px_0_20px_rgba(0,0,0,0.02)]">
        ...
        {/* 🟢 3. NODE STRENGTH WIDGET (New Integration) */}
        <div className="px-6 py-4 border-t border-slate-50 bg-slate-50/50">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Node Strength</span>
            <span className="text-[9px] font-black text-pink-600">{strength}%</span>
          </div>
          ...
          {strength < 100 && (
            <p className="text-[8px] text-slate-400 mt-2 italic">Restricted. Finish setup in Settings.</p>
          )}
        </div>
            ...
            <div className="flex items-center gap-6">
                <div className="flex flex-col text-right">
                    ...
                    <span className="text-sm font-mono font-bold text-slate-800">
                      TZS {user?.balance?.toLocaleString() ?? "0.00"}
                    </span>
                </div>
      ...
      {/* MOBILE MENU (Omitted for brevity, but stays same) */}
    </div>
  );
}
