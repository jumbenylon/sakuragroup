...
import { 
  ...
  Settings, LogOut, Bell, Zap, Menu, X
} from "lucide-react";
...
export default function AxisPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#050912] text-white font-sans selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-72 border-r border-white/5 bg-[#0B1120] flex-col z-50">
        <div className="p-8 border-b border-white/5">
          <div className="flex items-center gap-3 text-white">
             <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center shadow-lg shadow-purple-900/20">
                <Zap size={18} fill="currentColor" className="text-white" />
             </div>
             <div>
                <span className="font-black tracking-widest text-sm block">AXIS PORTAL</span>
                <span className="text-[9px] text-slate-500 font-mono uppercase tracking-widest">v2.4 Stable</span>
             </div>
          </div>
        </div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 px-4 py-3.5 rounded-xl text-sm font-bold transition-all group ${
                  isActive 
                    ? "bg-gradient-to-r from-white/10 to-transparent text-white border-l-2 border-emerald-500" 
                    : "text-slate-500 hover:text-white hover:bg-white/[0.02]"
                }`}
              >
                <link.icon size={18} className={`transition-colors ${isActive ? "text-emerald-500" : "text-slate-600 group-hover:text-white"}`} />
                {link.name}
              </Link>
            );
          })}
                 ...
                 <div className="space-y-4">
                  {navLinks.map((link) => (
                    <Link key={link.name} href={link.href} onClick={() => setMobileMenu(false)} className="flex items-center gap-4 p-4 bg-white/5 rounded-xl text-white font-bold">
                        <link.icon size={20} className="text-emerald-500"/> {link.name}
                    </Link>
                  ))}
                 </div>
            </motion.div>
      ...
      <main className="lg:pl-72 pt-16 lg:pt-0">
        <header className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-[#050912]/80 backdrop-blur-xl sticky top-0 z-40">
           <div className="hidden lg:flex items-center gap-4 text-slate-500 text-sm">
              <span className="font-mono text-xs text-emerald-500">● LIVE CONNECTION</span>
           ...
           <div className="flex items-center gap-8">
              <div className="text-right">
                 ...
                 <span className="font-mono text-xl font-bold text-white leading-none">
                    24,500 <span className="text-xs text-emerald-500">TZS</span>
                 </span>
              </div>
              ...
              <button className="p-2.5 bg-white/5 rounded-full hover:bg-white/10 text-slate-400 hover:text-white transition-colors relative border border-white/5">
                 <Bell size={18} />
                 <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-[#050912]" />
              </button>
           </div>
        </header>
