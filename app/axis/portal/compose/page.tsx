...
import { 
  ...
  Users, 
  Terminal, 
  MessageSquare, // 🟢 RESTORED: Fixed ReferenceError
  Calculator, 
  Loader2, 
  AlertCircle, 
  CheckCircle, 
...
/**
 * Axis Master Composer (v9.6)
 * Design: Sovereign Industrial / Tesla Minimalist.
 * Logic: High-volume dispatch with restored dependency tree for build success.
 */
...
export default function ComposePage() {
  ...
  const costPerSms = 21; // TZS Sakura Standard
  ...
  useEffect(() => {
    async function fetchSenders() {
      try {
        ...
        console.error("Infrastructure Handshake Failed", e);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            setStatus({ type: 'error', text: data.error || "Gateway Rejection." });
        }
    } catch (e) {
      ...
      <div className="grid lg:grid-cols-12 gap-10 items-start">
              ...
              <div className="bg-white border border-slate-100 p-10 rounded-[2.5rem] shadow-sm space-y-6">
                  ...
                  {loading ? (
                      ...
                      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                          {senderIds.length === 0 ? (
                             <div className="text-[10px] font-black text-slate-300 uppercase italic">No sender identities found.</div>
                          ) : senderIds.map((sid) => (
                              <button
                                key={sid.id}
              ...
              <div className="bg-white border border-slate-100 p-10 rounded-[2.5rem] shadow-sm space-y-6">
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                        <div className="flex gap-6">
                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">CHARS: <span className="text-slate-900">{charCount}</span></div>
                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">PARTS: <span className="text-slate-900">{smsCount}</span></div>
                        </div>
                        <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">GSM 03.38 Handshake</div>
                    </div>
              </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6 sticky top-24">
              ...
              <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] flex items-center gap-4">
                 ...
                 <p className="text-[10px] text-slate-400 font-bold italic leading-tight">Handshake verified. 98.2% predicted delivery success across carrier routes.</p>
              </div>
          </div>
      </div>
    </div>
